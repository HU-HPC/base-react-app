...
alter table semester owner to "user";
...
alter table instructor owner to "user";
...
alter table location owner to "user";

create table performance_indicators
(
	id bigserial not null
		constraint performance_indicators_pkey
			primary key,
	code text,
	description text
);

alter table performance_indicators owner to "user";
...
alter table course owner to "user";

create table course_prerequisite
(
	course_id bigserial not null
		constraint course_prerequisite_course_id_fkey
			references course,
	prerequisite_id bigserial not null
		constraint course_prerequisite_prerequisite_id_fkey
			references course
);

alter table course_prerequisite owner to "user";
...
(
	...
	course_id bigserial not null
		constraint assignments_course_id_fkey
			references course,
	semester_id bigserial not null
		constraint assignments_semester_id_fkey
			references semester,
	name text,
	rubric bytea
);

alter table assignments owner to "user";

create table course_semester
(
	course_id bigserial not null
		constraint course_semester_pi_course_id_fkey
			references course,
	semester_id bigserial not null
		constraint course_semester_pi_semester_id_fkey
			references semester,
	pi_id bigserial not null
		constraint course_semester_pi_pi_id_fkey
			references performance_indicators,
	instructor_id integer not null,
	id serial not null
		constraint course_semester_pk
			primary key
);

alter table course_semester owner to "user";

create table course_assessment
(
	id bigserial not null
		constraint course_assessment_pkey
			primary key,
	peo_id bigserial not null,
	course_semester_id integer not null
		constraint course_assessment_course_semester_id_fk
			references course_semester
);

alter table course_assessment owner to "user";

create unique index course_semester_id_uindex
	on course_semester (id);

create table course_schedule
(
	course_id bigserial not null
		constraint course_schedule_course_id_fkey
			references course,
	semester_id bigserial not null
		constraint course_schedule_semester_id_fkey
			references semester,
	week text,
	content text,
	deliverable text,
...
alter table course_schedule owner to "user";

create table class
(
	course_id bigserial not null
		constraint class_course_id_fkey
			references course,
	semester_id bigserial not null
		constraint class_semester_id_fkey
			references semester,
	instructor_id bigserial not null
		constraint class_instructor_id_fkey
			references instructor,
	location text,
	room_number integer,
	start_date date,
	...
	program_educational_objectives text,
...
alter table class owner to "user";

create table research_proposal
(
	id serial not null
		constraint research_proposal_pk
			primary key,
	faculty_id integer not null
		constraint research_proposal_instructor_id_fk
			references instructor,
	research_title text not null,
	description text not null,
	rationale text not null,
	design text not null,
	preliminary_data text not null,
	expected_results text not null,
	time_schedule text,
	qualifications text,
	sponsors text not null,
	consecutive_funding text not null,
	detailed_funding text not null,
	proposal_title text not null,
	deleted boolean default false not null
);

alter table research_proposal owner to "user";

create unique index research_proposal_id_uindex
	on research_proposal (id);
...
create function pg_catalog.pg_available_extension_versions(out name name, out version text, out superuser boolean, out relocatable boolean, out schema name, out requires name[], out comment text) returns setof record
	stable
	strict
	cost 10
	rows 100
...
create function pg_catalog.pg_get_keywords(out word text, out catcode "char", out catdesc text) returns setof record
	stable
	strict
	cost 10
	rows 400
...
create function pg_catalog.pg_get_replication_slots(out slot_name name, out plugin name, out slot_type text, out datoid oid, out temporary boolean, out active boolean, out active_pid integer, out xmin xid, out catalog_xmin xid, out restart_lsn pg_lsn, out confirmed_flush_lsn pg_lsn) returns setof record
	stable
	cost 1
	rows 10
	language internal
...
create function pg_catalog.pg_hba_file_rules(out line_number integer, out type text, out database text[], out user_name text[], out address text, out netmask text, out auth_method text, out options text[], out error text) returns setof record
	strict
	cost 1
	language internal
as $$
...
create function pg_catalog.pg_logical_slot_get_binary_changes(slot_name name, upto_lsn pg_lsn, upto_nchanges integer, options text[] default '{}'::text[], out lsn pg_lsn, out xid xid, out data bytea) returns setof record
	cost 1000
	language internal
as $$
begin
...
create function pg_catalog.pg_logical_slot_get_changes(slot_name name, upto_lsn pg_lsn, upto_nchanges integer, options text[] default '{}'::text[], out lsn pg_lsn, out xid xid, out data text) returns setof record
	cost 1000
	language internal
as $$
begin
...
create function pg_catalog.pg_logical_slot_peek_binary_changes(slot_name name, upto_lsn pg_lsn, upto_nchanges integer, options text[] default '{}'::text[], out lsn pg_lsn, out xid xid, out data bytea) returns setof record
	cost 1000
	language internal
as $$
begin
...
create function pg_catalog.pg_logical_slot_peek_changes(slot_name name, upto_lsn pg_lsn, upto_nchanges integer, options text[] default '{}'::text[], out lsn pg_lsn, out xid xid, out data text) returns setof record
	cost 1000
	language internal
as $$
begin
...
create function pg_catalog.pg_prepared_statement(out name text, out statement text, out prepare_time timestamp with time zone, out parameter_types regtype[], out from_sql boolean) returns setof record
	stable
	strict
	cost 1
	language internal
...
create function pg_catalog.pg_stop_backup(exclusive boolean, wait_for_archive boolean default true, out lsn pg_lsn, out labelfile text, out spcmapfile text) returns setof record
	strict
	cost 1
	language internal
as $$
...
create function pg_catalog.pg_timezone_abbrevs(out abbrev text, out utc_offset interval, out is_dst boolean) returns setof record
	stable
	strict
	cost 1
	language internal
